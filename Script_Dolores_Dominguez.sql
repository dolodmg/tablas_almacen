CREATE SCHEMA Almacenes;
USE Almacenes;

CREATE TABLE Provincias (
	idProvincia INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
	nombre VARCHAR(30) NOT NULL
);

CREATE TABLE Ciudades (
	codPostal VARCHAR(30) NOT NULL PRIMARY KEY,
    nombre VARCHAR(30) NOT NULL,
    idProvincia INT,
    CONSTRAINT fk_ciudad_prov FOREIGN KEY (idProvincia) REFERENCES Provincias(idProvincia)
);

CREATE TABLE Direcciones (
	numDireccion INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    calle INT NOT NULL,
    numero INT NOT NULL,
    codPostal VARCHAR(30) NOT NULL,
    CONSTRAINT fk_direc_ciudad FOREIGN KEY (codPostal) REFERENCES Ciudades(codPostal)
);

CREATE TABLE Almacenes (
  idAlmacen INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  nombre VARCHAR(30) NOT NULL,
  numDireccion INT NOT NULL,
  CONSTRAINT fk_almacen_direc FOREIGN KEY (numDireccion) REFERENCES Direcciones(numDireccion)
);

CREATE TABLE Horarios (
	idHorario INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    horarioApertura TIME NOT NULL,
    horarioCierre TIME NOT NULL,
    diaSemana VARCHAR(30)
);

CREATE TABLE HorariosAlmacen (
	idAlmacen INT NOT NULL,
    idHorario INT NOT NULL,
    PRIMARY KEY (idAlmacen,idHorario),
    CONSTRAINT fk_horaAl_almacen FOREIGN KEY (idAlmacen) REFERENCES Almacenes(idAlmacen),
    CONSTRAINT fk_horaAl_hora FOREIGN KEY (idHorario) REFERENCES Horarios(idHorario)
);

CREATE TABLE Proveedores (
	idProveedor INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    nombre VARCHAR(40) NOT NULL,
    telefono VARCHAR(40) NOT NULL,
    numDireccion INT NOT NULL,
    CONSTRAINT fk_proveedor_direccion FOREIGN KEY (numDireccion) REFERENCES Direcciones(numDireccion)
);
    
CREATE TABLE Articulos (
	codigoArticulo INT NOT NULL PRIMARY KEY,
    nombre VARCHAR(40) NOT NULL,
    precio FLOAT NOT NULL,
    stock INT NOT NULL,
    descripcion VARCHAR(50) NOT NULL
);    

CREATE TABLE Inventarios (
	idInventario INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    idAlmacen INT NOT NULL,
    CONSTRAINT fk_inventario_almacen FOREIGN KEY (idAlmacen) REFERENCES Almacenes(idAlmacen)
);

CREATE TABLE InventariosArticulos (
	idInventario INT NOT NULL,
    codigoArticulo INT NOT NULL,
    PRIMARY KEY (idInventario,codigoArticulo),
    CONSTRAINT fk_invArticulo_inventario FOREIGN KEY (idInventario) REFERENCES Inventarios(idInventario),
    CONSTRAINT fk_invArticulo_articulo FOREIGN KEY (codigoArticulo) REFERENCES Articulos(codigoArticulo)
);

CREATE TABLE Compras (
	numCompra INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    montoTotal FLOAT NOT NULL,
    fecha DATE NOT NULL,
    idProveedor INT NOT NULL,
    idAlmacen INT NOT NULL,
    CONSTRAINT fk_compra_proveedor FOREIGN KEY (idProveedor) REFERENCES Proveedores(idProveedor),
    CONSTRAINT fk_compra_almacen FOREIGN KEY (idAlmacen) REFERENCES Almacenes(idAlmacen)
);
    
CREATE TABLE ItemCompra (
	idItem INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    cantidad INT NOT NULL,
    precioUnidad FLOAT NOT NULL,
    codigoArticulo INT,
    numCompra INT,
    CONSTRAINT fk_itemC_articulo FOREIGN KEY (codigoArticulo) REFERENCES Articulos(codigoArticulo),
    CONSTRAINT fk_itemC_compra FOREIGN KEY (numCompra) REFERENCES Compras(numCompra)
);

CREATE TABLE Clientes (
	numCliente INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    nombre VARCHAR(30) NOT NULL,
    apellido VARCHAR(30) NOT NULL,
    dni INT NOT NULL,
    telefono VARCHAR(40) NOT NULL,
    numDireccion INT NOT NULL,
    CONSTRAINT fk_cliente_direccion FOREIGN KEY (numDireccion) REFERENCES Direcciones(numDireccion)
);

CREATE TABLE Ventas (
	numVenta INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    precioTotal FLOAT NOT NULL,
    fecha DATE NOT NULL,
    numCliente INT NOT NULL,
    idAlmacen INT NOT NULL,
    CONSTRAINT fk_venta_cliente FOREIGN KEY (numCliente) REFERENCES Clientes(numCliente),
    CONSTRAINT fk_venta_almacen FOREIGN KEY (idAlmacen) REFERENCES Almacenes(idAlmacen)
);

CREATE TABLE ItemVentas (
	idItem INT NOT NULL,
    cantidad INT NOT NULL,
    precioUnidad FLOAT NOT NULL,
    numVenta INT NOT NULL,
    codigoArticulo INT NOT NULL,
    CONSTRAINT fk_itemV_venta FOREIGN KEY (numVenta) REFERENCES Ventas(numVenta),
    CONSTRAINT fk_itemV_articulo FOREIGN KEY (codigoArticulo) REFERENCES Articulos(codigoArticulo)
);

CREATE TABLE Empleados (
	cuitEmpleado INT NOT NULL PRIMARY KEY,
    nombreApellido VARCHAR(50) NOT NULL,
    dni INT NOT NULL,
    telefono VARCHAR(40) NOT NULL,
    mail VARCHAR(50) NOT NULL,
    numDireccion INT NOT NULL,
    idAlmacen INT NOT NULL,
    CONSTRAINT fk_empleado_direccion FOREIGN KEY (numDireccion) REFERENCES Direcciones(numDireccion),
    CONSTRAINT fk_empleado_almacen FOREIGN KEY (idAlmacen) REFERENCES Almacenes(idAlmacen)
);

CREATE TABLE Categorias (
	idCategoria INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    descripcion VARCHAR(40) NOT NULL
);

CREATE TABLE EmpleadosCategorias (
	idCategoria INT NOT NULL,
    cuitEmpleado INT NOT NULL,
    PRIMARY KEY (idCategoria,cuitEmpleado),
    CONSTRAINT fk_empleadoCat_cat FOREIGN KEY (idCategoria) REFERENCES Categorias(idCategoria),
    CONSTRAINT fk_empleadoCat_empleado FOREIGN KEY (cuitEmpleado) REFERENCES Empleados(cuitEmpleado)
);

CREATE TABLE Sueldos (
	numSueldo INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    montoNeto FLOAT NOT NULL,
    montoBruto FLOAT NOT NULL,
    cuitEmpleado INT NOT NULL,
    CONSTRAINT fk_sueldo_empleado FOREIGN KEY (cuitEmpleado) REFERENCES Empleados(cuitEmpleado)
);